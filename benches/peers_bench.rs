...
use aimsir::{model, peers_controller};
...
async fn peers_measurements(peers_num: usize) {
    ...
    for peer_id in [..peers_num] {
        neighbour_update.update.push(model::aimsir::Peer {
...
fn bench_peers(c: &mut Criterion) {
    ...
    c.bench_with_input(BenchmarkId::new("calc_aggregates", size), &size, |b, &s| {
        // Insert a call to `to_async` to convert the bencher to async mode.
        // The timing loops are the same as with the normal bencher.
        b.to_async(tokio::runtime::Runtime::new().unwrap())
            .iter(|| peers_measurements(s));
    });
}
